...
var nickADDR = key.addressOf("7ea61db64cdf9ed027cb8405d499700ffd046ad3");
...
readContractMeta = function()
{

	var ADDR = key.addressOf(document.getElementById('address').value);
	...
	if (metaident == 585546221227) {
		...
		document.getElementById('creator').value = creat;
...
promoteUser = function(){
	...
	var command = bytes.fromString("promote",32);
...
generateUserTable = function(){
	...
	for (var i = 0; i < size; i++) {
		...
		var priv = u256.sub(pointer,u256.value(1));
		
		userPrivs[i] = u256.toValue(eth.storageAt(nickADDR,priv));
		pointer = u256.add(pointer, u256.value(2));
	};
...
getAddress256FromNick = function(userNick){
	var nick256 = bytes.u256of(bytes.fromString(userNick));
	var address256 = eth.storageAt(nickADDR,nick256);
	return address256;
}

getAddressStrFromNick = function(userNick){
	return bytes.toString(u256.bytesOf(getAddress256FromNick());
}

getUserPrivFromNick = function(){
	var nick256 = bytes.u256of(bytes.fromString(userNick));
	var privAddr256 = sub(nick256,u256.value(1));
	var priv256 = eth.storageAt(nickADDR,privAddr256);
	return u256.toValue(priv256);
}
